...
<body>
	...
	<%
		...
		/*
			IPv4 : 127.0.0.1	   => 127_0_0_1   , 마침표가 보이면 4버전
			IPv6 : 0:0:0:0:0:0:0:1 => 0_0_0_0_0_0_0_1	
		*/
		
		// 파일명 : 작성IP_작성일.txt
		String filename = ip.replace('.', '_').replace(':', '_') + "_" + createdDate.replace("-", "") + ".txt";
		// String filename = ip.replaceAll("[.:]", "_") + "_" + createdDate.replace("-", "") + ".txt"; // replaceAll() 은 정규식 형식으로 다수의 데이터를 처리가능하다.
		
		// 파일 객체
		File file = new File(dir, filename); // dir에 filename 이름을 가진 파일을 만들겠다
