...
public class HelloServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "GET":
			doGet(request, response); // doGet메소드에 다시 던져진다
			break;
		case "POST":
	...
	/*
	 	4. doGet()
	 		...
	 		2) GET 방식의 요청의 예시, 메소드를 생략하면 GET방식
	 			(1) <a href="http://localhost:9090/01_servlet/HelloServlet"> //이 서블릿(자바파일) 실행해 주세요
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		response.getWriter().append("Served at: ").append(request.getContextPath()); // 실질적으로 이것만 동작한다고 보면 된다.
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// POST방식은 doPost()는 중간에 데이터를 받기만 하고 doGet()으로 넘겨준다 실상 doGet()만 있고 일믈 다 한다고 보면된다.
