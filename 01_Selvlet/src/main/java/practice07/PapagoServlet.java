...
public class PapagoServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		con.setDoOutput(true);  	// OutputStream으로 POST 데이터를 넘겨주겠다는 옵션 메소드
		DataOutputStream dos = new DataOutputStream(con.getOutputStream());
		
		// Papago API로 파라미터 보내기
		dos.write(params.getBytes()); // getBytes(): 스트링을 바이트배열로 바꿔서 보내주는 메소드, 
		dos.flush();				  // write메소드는 string으로 보내기가 안되므로 getbytes()를 써서 바이트배열로 보내주는거, 필요한 부분만 인코딩을 해준거다
		dos.close();
		
		// Papago API로부터 번역 결과를 받아 올 입력 스트림 생성
		BufferedReader reader = null;
		if(con.getResponseCode() == 200) {
			reader = new BufferedReader(new InputStreamReader(con.getInputStream())); // 문자 스트림으로 바꾸는 과정
		} else {
