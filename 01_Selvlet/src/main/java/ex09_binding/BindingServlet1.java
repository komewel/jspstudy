...
public class BindingServlet1 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 	1.stateless
		 		1) 상태없음
		 		2) 웹 페이지간의 이동 stateless한 이동이다
		 		3) 현재 페이지에는 이전 페이지의 정보가 없다.
		 		4) 리다이렉트방식이다(그냥이동하는거다 아무런 값을 가지고 넘어가지 않는다) <a href="">나 location.href="" 도 리다이렉트이다.
		 			포워드는 정보를 전달한다.
		 		
		 	2. binding
		 		1) 값을 저장할 수 있는 영역에 속성(Attribute)의 형태로 값을 저장하는 것을 말한다.
		 		2) 저장 영역은 서버(웹 서버)가 제공한다.(서버내의 공간, 데이터베이스를 쓴다 이런 개념이 아니다)
		 		3) 3개 영역
		 			(1) HttpServletRequest : 하나의 요청 내에서 값을 저장할 수 있다. (1회용), 아무나 쓸수 있는게 아니다 
		 		...
		 		4) 속성(Attribute) 관련 메소드
		 			(1) getAttribute('속성')	 : 값 가져오기 (캐스팅(형 변환)이 필요할 수 있다.)
		 			(2) setAttribute('속성', 값) : 값 저장하기 (object 타입으로 저장한다.)
		 			(3) removeAttribute('속성')  : 값 제거하기
		 	 		 
		 */
		// 저장하고 알아서 넘어갈거임 BindingServlet2로 
		...
		ServletContext ctx = request.getServletContext(); // application 자체가 주인이기때문에 메소드만 불러올수도 있다
		ctx.setAttribute("a", 300);
		
		// 페이지 이동하기
		
		// 1. HttpServletRequest의 전달이 없는 이동 : redirect, <a href="">, location.href=""
		//response.sendRedirect("/01_Servlet/BindingServlet2"); 포워드랑 같이 있으면 안되므로 주석 처리
		
		// 2. HttpServletRequest의 전달이 있는 이동 : forward, 서버 내부에서 움직이는거라 urlmapping만 적으면된다.
