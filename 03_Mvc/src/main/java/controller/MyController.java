...
public class MyController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String requestURI = request.getRequestURI();   // getrequestURI() 전체주소를 가져오는 기능을 한다. /* /03_Mvc/today.do */ 
		...
		MyService myService = null; // 아직 어떤 서비스가 들어올지 결정된게 없다
		
		// URLMapping에 따른 모델(서비스) 생성하기 
		// 이렇게 조건식으로 메소드 본문을 채워주면 된다
		switch(urlMapping) {
		case "/today.do": 
